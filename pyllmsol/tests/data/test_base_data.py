...
# @Last modified time: 2024-11-14 18:23:51
...
# @pytest.fixture
# def data():
#     MockBase = Mock(_Base, name="MockBase")
#     instance = MockBase(items=["Sample text"], item_type=_BaseData, fallback_type=str, tokenizer=MockLlamaTokenizer())
...
#     text = "Sample text"
#     # item = _BaseData(text, tokenizer=mock_transformers_tokenizer)
#     # data = _Base(items=[item], item_type=_BaseData, fallback_type=str, tokenizer=mock_transformers_tokenizer)
#     assert isinstance(data.items[0], _BaseData)
...
#     """Test that _process_item converts a string into an _BaseData object using a tokenizer."""
#     text = "Sample text"
#     # data = _Base(items=[text], item_type=_BaseData, fallback_type=str, tokenizer=mock_transformers_tokenizer)
#     assert isinstance(data.items[0], _BaseData)
#     assert data.items[0].text == text
#     assert data.items[0].tokens == [101, 102, 103]
...
#         _Base(items=[123], item_type=_BaseData, fallback_type=str, tokenizer=mock_transformers_tokenizer)


# def test_append_item_valid(mock_transformers_tokenizer):
#     """Test that _append_item correctly adds a new item of valid type."""
#     text = "New item text"
#     data = _Base(items=[], item_type=_BaseData, fallback_type=str, tokenizer=mock_transformers_tokenizer)
...
#         data = _Base(items=["text"], item_type=_BaseData, fallback_type=str, tokenizer=None)
#         data.tokenize("text")
...
#     data = _Base(items=["Sample text"], item_type=_BaseData, fallback_type=str, tokenizer=mock_transformers_tokenizer)
...
def test_base_data_initialization(mock_llama_tokenizer, mock_transformers_tokenizer):
    text = "This is a sample text for testing."
    data = _BaseData(text, tokenizer=mock_transformers_tokenizer)
...
def test_tokens_property(mock_llama_tokenizer, mock_transformers_tokenizer):
    data = _BaseData("Sample text", tokenizer=mock_transformers_tokenizer)
    assert data.tokens == [101, 102, 103]

    data = _BaseData("Sample text", tokenizer=mock_llama_tokenizer)
    assert data.tokens == [101, 102, 103]
...
def test_from_textfile(mock_llama_tokenizer, tmp_path):
    ...
    data = _BaseData.from_textfile(path, tokenizer=mock_llama_tokenizer)
    assert data.text == "File text content"


def test_add_operator(mock_transformers_tokenizer):
    data1 = _BaseData("Hello, ", tokenizer=mock_transformers_tokenizer)
    data2 = _BaseData("world!", tokenizer=mock_transformers_tokenizer)
    result = data1 + data2
...
def test_base_dataset_initialization(mock_transformers_tokenizer):
    text_list = ["Text one.", "Text two.", "Text three."]
    dataset = _BaseDataSet(text_list, batch_size=2, tokenizer=mock_transformers_tokenizer)
    assert len(dataset.items) == 3
    assert dataset.batch_size == 2


def test_base_dataset_iteration(mock_llama_tokenizer):
    text_list = ["Text one.", "Text two.", "Text three."]
    dataset = _BaseDataSet(text_list, batch_size=2, tokenizer=mock_llama_tokenizer)
...
def test_base_dataset_out_of_bounds_iteration(mock_transformers_tokenizer):
    text_list = ["Text one.", "Text two."]
    dataset = _BaseDataSet(text_list, batch_size=1, tokenizer=mock_transformers_tokenizer)
    iterator = iter(dataset)
    next(iterator)
    next(iterator)
...
def test_from_json(mock_transformers_tokenizer, tmp_path):
    ...
    dataset = _BaseDataSet.from_json(json_path, batch_size=1, tokenizer=mock_transformers_tokenizer)
...
def test_from_jsonl(mock_llama_tokenizer, tmp_path):
    ...
    dataset = _BaseDataSet.from_jsonl(jsonl_path, batch_size=1, tokenizer=mock_llama_tokenizer)
    assert len(dataset.items) == 2
